...
void Gale_diagram::convert_bin2vertices(Tpoint *diagram){
...
int Gale_diagram::read_diagram_bin(FILE *inf){
    ...
    int s = fread (diagram, sizeof(Tpoint), nverts, inf);
...
int Gale_diagram::write_diagram_bin(FILE *outf){
    ...
    for (int i = 0; i < nverts; i++){
        diagram[i] = 0;
        for (int j = 0; j < DIM; j++)
            diagram[i] |= (Tpoint)(vertices[i][j] + 1) << (j*2);
    }
    int s = fwrite (diagram, sizeof(Tpoint), nverts, outf);
    if (s < nverts){
