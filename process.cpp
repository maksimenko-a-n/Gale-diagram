...
// Generate array of all permutations of coordinates
vector< vector<char> > gen_permute(){
    //int nperm = 1;
    //for (int i = 2; i <= DIM; i++)  nperm *= i;
...
void permute_coord(Tpoint *output, Tpoint *input, int nv, char *permut){
    for (int i = 0; i < nv; i++){
        output[i] = 0;
        Tpoint x = input[i];
...
void lex_min(Tpoint *output, Tpoint *input, int size, vector< vector<char> > *all_permutations){
    int npermut = all_permutations->size();
    output[0] = ~(Tpoint)0;
    Tpoint current1[MAX_VERT], current2[MAX_VERT];
    for (int i = 0; i < npermut; i++){ // Choose permutation of coordinates
        ...
        for (int j = 0; j < nreflections; j++){ // Choose reflection
            Tpoint mask = 0; // mask for reflections
            Tpoint mask2 = 0b10; // one-bit reflection
            Tpoint mask3 = 1; // mask for zeroes
            int x, k;
...
void process_one_diagram(int old_nverts, Tpoint *diagram, Tpoint *buffer, int &num, Gale_diagram &gale, vector< vector<char> > *all_permutations){
    num = 0;
    ...
    Tpoint zero = 0; // The zero-point
    for (int k = 0; k < DIM; k++)
        zero |= ((Tpoint)1 << (k*2));
    // is_old_vertices[i] == 0  <=>  i is a new vertex and is not zero
    char is_old_vertices[MAX_Tpoint+1];
    memset (is_old_vertices, 0, MAX_Tpoint+1);
    ...
    for (int i = 0; i < Nall; i++){ // Look over all possible points
        ...
        if (is_write){
            ...
            //memcpy(cur_diagram+1, diagram, (old_nverts+1)*sizeof(Tpoint));
...
int write_results(FILE *outf, FILE *out2f, FILE *out2ftxt, int nverts, Tpoint *buffer, int num, long long &savednum, int &min2facets, int &minf, int &maxf, long long &polynum, long long *distribution){
    Tpoint *diagram = buffer;
    for (int n = 0; n < num; n++, diagram += nverts + 4){
        ...
        if (diagram[0] != 1){ // Will be used for the next generation
            ...
            int s = fwrite (diagram+1, sizeof(Tpoint), nverts, outf);
            if (s < nverts){
        ...
        if (diagram[0] != 0){ // Write special polytopes 
            ...
            int s = fwrite (diagram+1, sizeof(Tpoint), nverts, out2f);
            if (s < nverts){
                printf ("Write file ERROR: Cann't write %d bytes\n", nverts);
                return 2;
            }
...
// Print the left time to file f
// total -- the all amount of work
// processed -- the amount of done work
void fprint_time (FILE *f, clock_t start_t, clock_t cur_t, long long total, long long processed){
    double elapsed_time = (double)(cur_t - start_t) / CLOCKS_PER_SEC;
    double left_time = elapsed_time * (total - processed) / processed;
    long sec = round(left_time);
    int min = sec / 60;
    int hour = min / 60;
    fprintf (f, " (left %d:%02d:%02d)", hour, min%60, sec%60);
}                
...
int main(int argc, char *argv[])
{
    ...
    Tpoint diagram[MAX_VERT];
	Tpoint buffer[Nall * (nverts+5)]; // Every block has (nverts+5) numbers
    // The first number -- type: ordinary diagram (0) or special one (1) or dual 2-nghb (2)
	...
	if (rank != 0) {
		...
		while(1) {
			...
			if(rcv_get < 1) break;
			MPI_Recv(diagram, rcv_get*nverts*sizeof(Tpoint), MPI_BYTE, 0, FOR_PROCESSING_TAG, MPI_COMM_WORLD, &status);
            process_one_diagram(nverts, diagram, buffer, size, gale, &all_permutations); // The main process
			MPI_Isend(&size, 1, MPI_INT, 0, RESULT_READY_TAG, MPI_COMM_WORLD, &request);
            if (size > 0)
                MPI_Send(buffer, size*(nverts+5)*sizeof(Tpoint), MPI_BYTE, 0, RESULT_TAG, MPI_COMM_WORLD);
		}
    ...
    long long inputlen = ftell(inf) / (nverts * sizeof(Tpoint)); // The total number of diagrams
    ...
    long long savednum = 0; // The number of diagrams, saved in the main output file
	...
	clock_t begt = clock();
	// READ AND EVALUATE
	long long sent = 0; // amount of input which is currently read
	long long received = 0; // amount of output which is currently written
	...
	while(!end_of_file || to_slaves != from_slaves) {
		...
		if (rcv_get > 0){ // Receive results from worker
            MPI_Recv(buffer, rcv_get*(nverts+5)*sizeof(Tpoint), MPI_BYTE, status.MPI_SOURCE, RESULT_TAG, MPI_COMM_WORLD, &status);
        ...
        if (from_slaves%10000000 == 0)
            fprintf (logf, " Received %lld (%lld calls)", received, from_slaves);
        if (end_of_file){
            int send = 0;
		...
		int size;
		size = fread (diagram, sizeof(Tpoint), nverts, inf);
        ...
        size = 1;
        MPI_Isend(&size, 1, MPI_INT, status.MPI_SOURCE, TO_WORK_TAG, MPI_COMM_WORLD, &request);
		MPI_Send(diagram, size * nverts * sizeof(Tpoint), MPI_BYTE, status.MPI_SOURCE, FOR_PROCESSING_TAG, MPI_COMM_WORLD);
		to_slaves++;
		sent += size;
        if (sent == 100000 || sent % 10000000 == 0){
            fprintf (logf, " Sent %lld", sent);
            if (sent != 0)
                fprint_time (logf, begt, clock(), inputlen, sent);
            fflush (logf);
            fflush (outf);
        }    
    }
    ...
    fprintf (logf, "New number of diagrams = %lld\n", savednum);
    fprintf (logf, "min facets = %d, max facets = %d, polytopes num = %lld\n", minf, maxf, polynum);
    fprintf (logf, "Facets distr:");
    for (int i = 0; i <= MAX_FACET; i++)
        fprintf (logf, " d[%d] = %lld,", i, distribution[i]);
    fprintf (logf, "\n");
